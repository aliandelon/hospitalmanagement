...
<div class="user-roles-mapping-form">
	...
	<?= $form->field($model, 'user_id')->dropDownList(yii\helpers\ArrayHelper::map(common\models\AdminDetails::find()->where(['status' => 1])->all(), 'admin_id', 'name'), ['prompt' => 'Select Sub Super Admin']) ?>
        <?php if(!empty($tasks)){?>
            <div class="row">
                <label class="control-label" for="roles_id">Roles</label>
                <label>
                    <input type="checkbox" value="" id="selectall"> <i></i>Select All</label>
             </div>
          <div class="row">
            ...
            <?php foreach($tasks as $tsks){?>
                <div class="col-lg-2 col-md-3 col-sm-3 col-xs-12" style="min-height: 200px; !important;">
                    <div class="i-checks ">
                        <label>
    <input type="checkbox" value="<?=$tsks->task?>" class="<?='checkboxall main main'.$tsks->task?>">
     <i></i><?=($tsks->task) ? $taskArr[$tsks->task]:''?> 
                    ...
                    ?>
                      <div class="i-checks ">
                        <label>
                            <input type="checkbox" val="<?=$tsks->task?>" class="<?='checkboxall individual-check individual'.$tsks->task?>" value=""> <i></i><?=($subtask->sub_task)?$subtask->sub_task:""?>
                        </label>
                    </div>
                    <?php
                         }
